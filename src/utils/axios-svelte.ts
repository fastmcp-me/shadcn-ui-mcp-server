...
/**
 * Push component to the Svelte repository
...
async function pushComponentToRepository(params: {
    ...
    try {
        ...
        for (const file of files) {
            let filePath: string;
            let fileExtension: string = '.svelte';
            
            if (file.type === 'component') {
                filePath = `${REGISTRY_PATH}/ui/${componentName.toLowerCase()}/${componentName.toLowerCase()}${fileExtension}`;
            } else if (file.type === 'demo') {
                filePath = `${REGISTRY_PATH}/examples/${componentName.toLowerCase()}-demo${fileExtension}`;
            } else {
        ...
        if (createPullRequest && targetBranch !== branch) {
            const prData = {
                ...
                head: targetBranch,
                base: branch,
                body: `${commitMessage}\n\nThis PR adds the ${componentName} component to the Svelte registry.\n\nFiles added/updated:\n${pushedFiles.map(f => `- ${f.path}`).join('\n')}`
            };
