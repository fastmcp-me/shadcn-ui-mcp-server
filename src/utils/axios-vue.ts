...
/**
 * Push component to the Vue repository
...
async function pushComponentToRepository(params: {
  ...
  try {
    ...
    for (const file of files) {
      let filePath: string;
      let fileExtension: string = '.vue';
      
      if (file.type === 'component') {
        const formattedComponentName = formatComponentNameToCapital(componentName);
        const componentFolder = componentName.toLowerCase();
        filePath = `${UI_PATH}/${componentFolder}/${formattedComponentName}${fileExtension}`;
      } else if (file.type === 'demo') {
        const formattedComponentName = formatComponentNameToCapital(componentName);
        filePath = `${DEMOS_PATH}/${formattedComponentName}Demo${fileExtension}`;
      } else {
    ...
    if (createPullRequest && targetBranch !== branch) {
      const prData = {
        ...
        head: targetBranch,
        base: branch,
        body: `${commitMessage}\n\nThis PR adds the ${componentName} component to the Vue registry.\n\nFiles added/updated:\n${pushedFiles.map(f => `- ${f.path}`).join('\n')}`
      };
